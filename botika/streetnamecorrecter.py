...
def selectstreets(
    ...
    cursor.execute(q)
    logging.info("Fixing {0}".format(q))
    try:
...
def applychanges(
    ...
    onequery : query when there is no need for clustering
    kmeansquery : query when there are too much data and needs to be clustered
    operation : function that gets the old record, the new one and a
    replacement, returns None if the change does not apply and the new record
    when does
    """
    cant = 0
    ...
    for k, v in replacements.iteritems():
        ...
        for group in mydict.values():
            if len(group):
                CHANGESETAUTOTAGS[u'comment'] = (
                    basecomment + u"({0})".format(v)
                )
                l = client.ChangesetCreate(CHANGESETAUTOTAGS)
                logging.info(u"Processing changeset {0} : {1} -> {2} ".format(
                    l, k, v
                ))
                cont = 0
                for idw in group:
...
def fixstreetname(oldw, updw, replacement):
    ...
    """
    updw[u'tag'][u'name'] = (
        replacement + updw[u'tag'][u'name'][updw[u'tag'][u'name'].index(" "):]
    )
...
def fixwaynames():
    ...
    applychanges(
        ...
        client,
        replacements,
        "SELECT count(*) as cant FROM ways WHERE "
        "tags ?& Array['highway','name'] AND tags -> 'name' ~* '^({0}) .*';",
        "SELECT 1, id FROM ways WHERE tags ?& Array['highway','name'] AND "
        "tags -> 'name' ~* '^({1}) .*';",
        "SELECT kmeans(ARRAY[ST_X(l.p), ST_Y(l.p)], {0}) OVER (), "
        "l.id, l.name FROM (SELECT id,tags -> 'name' AS name, "
        "ST_PointN(linestring,1) AS p FROM ways WHERE tags ?& "
        "Array['highway','name'] AND tags -> 'name' ~* '^({1}) .*') AS l "
        "ORDER BY kmeans,name;",
        fixstreetname,
