...
psql -c "SELECT id, tags -> 'place' AS place, tags -> 'name' AS name, st_y(geom) AS lat, st_x(geom) AS lon, geom INTO places FROM nodes WHERE tags -> 'place' IN ('city', 'town', 'village') AND upper(tags -> 'name') NOT SIMILAR TO '(VEREDA|VDA|RESGUARDO|[0-9]%)%' ORDER BY tags -> 'name';"
