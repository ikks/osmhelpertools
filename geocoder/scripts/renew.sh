...
wget "http://download.geofabrik.de/openstreetmap/south-america/"$COUNTRY"-latest.osm.bz2"
time bunzip2 $COUNTRY"-latest.osm.bz2"
dropdb $DBNAME
createdb -O $DBUSER -T template_postgis $DBNAME
time osm2pgsql -d $DBNAME -l -s -p $DBUSER $COUNTRY"-latest.osm"
...
time $OSMOSIS --read-xml $COUNTRY"-latest.osm" --write-pgsql database=$DBNAME user=$DBSUPR host=$DBHOST password=$DBPSUP
...
psql -c "SELECT id, tags -> 'place' AS place, tags -> 'name' AS name, st_y(geom) AS lat, st_x(geom) AS lon, geom INTO places FROM nodes WHERE tags -> 'place' IN ('city', 'town', 'village') AND upper(tags -> 'name') NOT SIMILAR TO '(VEREDA|VDA|RESGUARDO|[0-9]%)%' ORDER BY tags -> 'name';" $DBNAME
...
rm $COUNTRY"-latest.osm"
redis-cli flushdb
python load_data.py


# Extract from extract.bbike.org
# w3m http://extract.bbbike.org/?lang=en&sw_lng=-81.875&sw_lat=12.429&ne_lng=-81.211&ne_lat=13.43&format=osm.xz&email=igor%40tamarapatino.org&as=0.04513671875&pg=1&coords=&layers=B000000T&city=San+Andr%C3%A9s+Islas%2C+Colombia&submit=extract
